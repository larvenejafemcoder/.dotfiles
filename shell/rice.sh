...
# ── Terminal theme (Catppuccin for GNOME Terminal) ────────────
if command -v python3 &>/dev/null; then
    info "Installing Catppuccin GNOME Terminal theme..."
    TMP_FILE="$(mktemp)"
    if curl -fsSL "https://raw.githubusercontent.com/catppuccin/gnome-terminal/v1.0.0/install.py" -o "$TMP_FILE"; then
        python3 "$TMP_FILE" && info "Catppuccin GNOME Terminal theme applied" \
            || warn "Catppuccin GNOME Terminal theme install had issues"
    else
        warn "Failed to download Catppuccin installer"
    fi
    rm -f "$TMP_FILE"
fi
